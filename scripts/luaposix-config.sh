...
# would normally produce.  Same idea as autoconf's AC_CHECK_HEADERS /
# AC_CHECK_FUNCS, zero dependencies beyond cc: compile (header probes) or
# compile+link (function probes) a minimal program; success => #define.
# Function probes try -lcrypt first (crypt(3) on glibc), then without.
#
# Also emits two fixups that must come AFTER the probes:
#
#   macOS _POSIX_TIMERS: <unistd.h> unconditionally defines it as -1 (no
#   timer_create etc.) even though clock_gettime has worked since 10.12,
#   and luaposix gates clock_gettime on it.  Simply defining a new value
#   wouldn't survive the source file's own #include <unistd.h> -- so this
#   header includes <unistd.h> itself (consuming the include guard), then
#   overrides.  The later include becomes a no-op and our value sticks.
#
#   LPOSIX_2001_COMPLIANT: gates ctype.c, sched.c, sys/socket.c, parts of
#   unistd.c (mirrors ext/include/_helpers.c).  Derived after <unistd.h>
#   so _POSIX_VERSION is visible.
set -eu
